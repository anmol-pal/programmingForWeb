...
export default function makeAuthServices(dao, rounds=DEFAULT_BCRYPT_ROUNDS) {
...
class AuthServices {
  ...
  async remove(params) {
    const chk = this.checker.validate('remove', params);
    if (chk.errors) return chk;
    const { userId } = params;
  ...
  async update(params) {
    ...
    return await this.dao.update(params.userId, updates);
  }
